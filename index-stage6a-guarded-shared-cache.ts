...
type SharedContentCacheState = "BYPASS" | "MISS" | "HIT" | "MISS-NOT-STORED";

function contentSharedCacheEnabled(env: any): boolean {
  const value = String(env.CONTENT_SHARED_CACHE_ENABLED || "").trim().toLowerCase();
  return value === "1" || value === "true" || value === "yes" || value === "on";
}

function sharedContentCacheTtlSeconds(url: URL): number {
  if (url.pathname === "/internal/content-read/lectures") return 30;
  if (url.pathname.startsWith("/internal/content-read/lectures/")) return 60;
  if (url.pathname === "/internal/content-read/materials-data") return 30;
  if (url.pathname === "/internal/content-read/mottos/active") return 60;
  if (url.pathname === "/internal/content-read/calendar/global") return 30;
  if (url.pathname === "/internal/content-read/search") return 30;
  return 15;
}
...
function canonicalSharedContentCacheUrl(url: URL): URL {
  ...
  if (url.pathname === "/internal/content-read/lectures") {
    for (const key of ["mainSubject", "subSubject", "trackMode", "department"]) {
      const value = url.searchParams.get(key);
      if (value && value.trim()) {
        canonical.set(key, value.trim().toLowerCase());
      }
    }
  } else if (url.pathname === "/internal/content-read/materials-data") {
    const requested = url.searchParams.get("scope");
    const scope =
      requested === "subjects" || requested === "offline"
        ? requested
        : "full";
    canonical.set("scope", scope);
  } else if (url.pathname === "/internal/content-read/search") {
    const q = url.searchParams.get("q");
    if (q && q.trim()) canonical.set("q", q.trim().toLowerCase());
  }

  cacheUrl.search = canonical.toString();
  return cacheUrl;
}

function responseWithSharedCacheState(
  response: Response,
  state: SharedContentCacheState,
  ttlSeconds?: number,
): Response {
  const headers = new Headers(response.headers);
  headers.set("Cache-Control", "no-store");
  headers.set("X-Content-Cache", state);

  if (typeof ttlSeconds === "number") {
    headers.set("X-Content-Cache-TTL", String(ttlSeconds));
  } else {
    headers.delete("X-Content-Cache-TTL");
  }
...
async function withSharedContentCache(
  ...
  // Authentication is deliberately performed by handleInternalContentRead()
  ...
  const ttlSeconds = sharedContentCacheTtlSeconds(url);
  const cacheKeyUrl = canonicalSharedContentCacheUrl(url);
  const cacheKey = new Request(cacheKeyUrl.toString(), { method: "GET" });
  const cache = caches.default;

  const cached = await cache.match(cacheKey);
  if (cached) {
    return responseWithSharedCacheState(cached, "HIT", ttlSeconds);
  }

  const response = await producer();

  // Only successful shared JSON reads are cacheable. Authentication failures,
  // validation errors, 404s, and server errors are NEVER stored.
  if (response.status !== 200) {
    return responseWithSharedCacheState(response, "MISS-NOT-STORED", ttlSeconds);
  }

  const cacheHeaders = new Headers(response.headers);
  cacheHeaders.set("Cache-Control", `public, max-age=${ttlSeconds}`);
  cacheHeaders.delete("X-Content-Cache");
  cacheHeaders.delete("X-Content-Cache-TTL");
  ...
  return responseWithSharedCacheState(response, "MISS", ttlSeconds);
}
...
async function handleInternalContentRead(request: Request, env: any, url: URL): Promise<Response> {
  ...
  if (url.pathname === "/internal/content-read/lectures") {
    return withSharedContentCache(
      request,
      env,
      url,
      () => readLectureListFromD1(url, env),
    );
  }
  ...
  if (url.pathname === "/internal/content-read/materials-data") {
    return withSharedContentCache(
      ...
      env,
      url,
      () => readMaterialsDataFromD1(url, env),
    );
  }
  ...
  if (match) {
    try {
      const id = validateReadId(match[1]);
      return withSharedContentCache(
        request,
        env,
        url,
        () => readLectureDetailFromD1(id, env),
      );
    } catch (error) {
      return jsonNoStore(
    ...
    if (url.pathname === "/health") {
      try {
        ...
        return Response.json({
          ...
          sharedContentCache: {
            enabled: contentSharedCacheEnabled(env),
            mode: contentSharedCacheEnabled(env) ? "cloudflare-cache-api" : "disabled",
          },
