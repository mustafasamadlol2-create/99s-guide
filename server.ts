...
// Academic PDFs (lecture slides, notes, handouts) rarely exceed 100 MB.
// Bounding upload size keeps multer's streaming write, the DB bytea write,
// and the in-memory buffer used to persist to PostgreSQL within reasonable limits.
const MAX_PDF_UPLOAD_BYTES = 100 * 1024 * 1024; // 100 MB
...
app.get("/api/lectures", requireUser, catchAsync(async (req, res) => {
  try {
    ...
    if (!forceRefresh && lecturesCache && lecturesCache.expiresAt > Date.now()) {
    ...
    lecturesCache = { data: lectures, expiresAt: Date.now() + 30_000 };
    res.json(lectures);
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
...
app.post("/api/materials/upload", requireAdmin, uploadLimiter, (req: any, res: any, next: any) => {
  ...
  try {
    ...
    // One active PDF/NOTE slot per lecture. Replacement is performed in one
    // transaction so catalog reads cannot observe a half-written material set.
    const replacedMaterials = await prismaClient.material.findMany({
      where: { lectureId, type },
      select: { fileUrlOrLink: true },
    });
    ...
    for (const previous of replacedMaterials) {
      if (previous.fileUrlOrLink?.startsWith("/uploads/materials/")) {
        ...
        try { await fs.unlink(previousPath); } catch (e: any) {
          if (e?.code !== "ENOENT") logger.warn("[Storage]", "Failed to remove replaced material file.");
        }
      }
    }
  ...
  try {
    ...
    if (!material.fileData) {
      if (material.fileUrlOrLink && material.fileUrlOrLink !== `/api/materials/pdf/${idToFind}`) {
        // Only redirect to same-origin/relative storage paths. Never turn a DB
        // field into an arbitrary external redirect from an authenticated route.
...
app.get("/api/materials", requireUser, catchAsync(async (req, res) => {
  try {
    ...
    if (materialsCache && materialsCache.group === currentUserGroup && !forceRefresh) {
    ...
    const [dbLectures, dbMcqs, dbFlashcards, dbMaterials, dbEvents] = await Promise.all([
      ...
      prismaClient.mcq.findMany({ take: 2000 }),
      prismaClient.flashcard.findMany({ take: 2000 }),
      prismaClient.material.findMany({ take: 2000, select: { id: true, title: true, type: true, fileUrlOrLink: true, lectureId: true } }),
      prismaClient.calendarEvent.findMany({ take: 2000, where: { userId: null } })
    ]);
    ...
    materialsCache = { group: String(currentUserGroup || "").trim().toUpperCase(), data: responseData };
