...
// ── Issued OAuth state tokens (CSRF / sandbox-bypass prevention) ─────────────
// Every state token generated by /api/auth/oauth-url is recorded here with a
// 10-minute TTL.  The callback validates the incoming state against this map
// and removes it (one-time use) to prevent replay.  This also closes the
// sandbox-bypass window: an attacker cannot call /auth/callback/sandbox with
// a crafted email unless they first obtained a legitimate state token from our
// server — which in turn proves the request originated from the app UI.
const pendingOAuthStates = new Map<string, number>(); // stateToken → expiresAt
const _oauthStatesCleanup = setInterval(() => {
  const now = Date.now();
  for (const [k, exp] of pendingOAuthStates.entries()) {
    if (exp < now) pendingOAuthStates.delete(k);
  }
}, 10 * 60 * 1000);
if (typeof _oauthStatesCleanup.unref === "function") _oauthStatesCleanup.unref();
...
io.on("connection", (socket) => {
  ...
  if (socketToken) {
    try {
      const payload = jwt.verify(socketToken, JWT_SECRET, { algorithms: ["HS256"] }) as { userId: string };
...
function setCookieToken(res: any, userId: string, email: string): string {
  const token = jwt.sign({ userId, email }, JWT_SECRET, { algorithm: "HS256", expiresIn: "30d" });
  res.cookie("auth_token", token, {
    httpOnly: true,
    secure: true,      // Required for SameSite=None
    sameSite: "none",  // Allows cross-origin cookie sending for the PWA / Capacitor context
...
async function requireAdmin(req: express.Request, res: express.Response, next: express.NextFunction) {
  try {
    ...
    const decoded = jwt.verify(token, JWT_SECRET, { algorithms: ["HS256"] }) as { userId: string; email: string; iat?: number };
...
app.get("/api/auth/oauth-url", (req, res) => {
  ...
  pendingOAuthStates.set(stateToken, Date.now() + 10 * 60 * 1000);
  ...
  // Note: stateToken is also recorded in pendingOAuthStates below
  if (provider === "apple") {
...
app.get("/auth/callback/sandbox", catchAsync(async (req, res) => {
  // Block in production: real OAuth credentials must be configured, and this
  // endpoint would otherwise allow authentication bypass via crafted emails.
  if (process.env.NODE_ENV === "production") {
  ...
  if (!sandboxStateToken || !pendingOAuthStates.has(sandboxStateToken)) {
    return res.status(403).send("Invalid or expired state token. Please sign in again.");
  }
  pendingOAuthStates.delete(sandboxStateToken); // one-time use
...
app.get(["/auth/callback/:provider", "/auth/callback/:provider/"], catchAsync(async (req, res) => {
  ...
  if (!errorParam) {
    if (!stateToken || !pendingOAuthStates.has(stateToken)) {
      if (isRedirectFlow) {
        return res.redirect(`/?oauth_error=${encodeURIComponent("Invalid or expired state token")}`);
      }
    ...
    pendingOAuthStates.delete(stateToken); // one-time use
  }
  ...
  try {
    ...
    if (provider === "google") {
      ...
      const response = await fetch(tokenUrl, {
        ...
        signal: AbortSignal.timeout(10_000), // 10 s — prevents hanging workers
      });
