...
export const CommandPalette = memo(function CommandPalette({
 ...
 try {
  ...
  try {
  const response = await apiClient(`/api/search?q=${encodeURIComponent(value.trim())}`, {
  ...
  fetchedResults = payload.filter((item): item is SearchResultItem =>
  !!item && typeof item === "object" && typeof (item as SearchResultItem).id === "string" && typeof (item as SearchResultItem).title === "string"
  );
  }
 }
 } catch (err: any) {
 ...
 // Merge avoiding duplicates
  const seen = new Set(fetchedResults.map((r) => r.id));
  localResults.forEach((lr: SearchResultItem) => {
 if (!seen.has(lr.id)) {
 ...
 className={`cp-result-item w-full flex items-center gap-3 px-4 py-3 cursor-pointer transition-all duration-200 group mx-2 max-w-[calc(100%-1rem)] rounded-xl active:scale-[0.98] active:opacity-90 ${
 isSelected
 ...
 <ChevronRight className={`cp-item-chevron w-icon-sm h-icon-sm text-neutral-500 dark:text-[#EBEBF599] transition-opacity ${ isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-80" }`} />
 </div>
 );
 };
 ...
 className="cp-clear-btn p-1 rounded-full text-neutral-400 dark:text-white/40 hover:text-neutral-600 dark:hover:text-white/70 transition-colors shrink-0 relative z-10"
 >
