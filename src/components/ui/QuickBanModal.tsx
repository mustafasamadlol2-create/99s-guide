/**
 * QuickBanModal — modern full-page centered overlay for banning users from Live Study Hall.
 * Reuses the same UserBan table + accountStatus + socket infrastructure
 * as the Moderation penalty system. No duplicate backend logic.
 */
import React, { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "motion/react";
import { X, ShieldOff, Loader2, Infinity as InfinityIcon, Clock } from "lucide-react";
import { apiClient } from "../../core/api/apiClient";
import { UserAvatar } from "../../features/profile/components/UserAvatar";
import { FormError } from "./FormError";

export interface QuickBanTarget {
  userId: string;
  name: string;
  email: string;
  avatarUrl?: string;
...
export const QuickBanModal: React.FC<QuickBanModalProps> = ({ target, onClose, onSuccess }) => {
  ...
  const handleSubmit = useCallback(async () => {
    if (!target) return;
    if (!reason.trim()) { setError("Please enter a reason for the ban."); return; }
    ...
    setError("");
    try {
      const res = await apiClient("/api/moderation/bans", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setError(data.error || "Ban failed. Please try again.");
        return;
      }
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
            <motion.div
              ...
              className="pointer-events-auto w-full max-w-md bg-white dark:bg-[#1C1C1E] rounded-2xl shadow-2xl border border-neutral-200/60 dark:border-white/[0.08] overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="flex items-start justify-between px-6 pt-6 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-neutral-100 dark:bg-white/[0.06] border border-neutral-200/60 dark:border-white/[0.08] flex items-center justify-center shrink-0">
                    <ShieldOff className="w-4.5 h-4.5 text-neutral-600 dark:text-neutral-300" />
                  </div>
                  <div>
                    ...
                    <p className="text-[12px] text-neutral-500 dark:text-neutral-400 mt-0.5">
                      This will suspend the account immediately.
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    maxLength={300}
                    className="w-full bg-neutral-50 dark:bg-white/[0.04] border border-neutral-200 dark:border-white/[0.08] rounded-xl px-4 py-3 text-[14px] text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-neutral-600 outline-none focus-visible:ring-2 focus-visible:ring-neutral-400/30 dark:focus-visible:ring-white/20 focus-visible:border-neutral-300 dark:focus-visible:border-white/20 resize-none transition"
                  />
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <InfinityIcon className={`w-4 h-4 ${isPermanent ? "text-rose-500 dark:text-rose-400" : "text-neutral-400 dark:text-neutral-500"}`} />
                    ...
                    <div className={`w-10 h-5.5 rounded-full transition-colors relative ${isPermanent ? "bg-rose-500" : "bg-neutral-200 dark:bg-neutral-700"}`}
                      ...
                      >
                        <div className="flex items-center gap-2 pt-0.5">
                          ...
                          <select
                            ...
                            className="flex-1 appearance-none bg-neutral-50 dark:bg-white/[0.04] border border-neutral-200 dark:border-white/[0.08] rounded-xl px-3 py-2.5 text-[14px] text-neutral-900 dark:text-white outline-none focus-visible:ring-2 focus-visible:ring-neutral-400/30 dark:focus-visible:ring-white/20 transition cursor-pointer"
                          >
              ...
              <div className="px-6 py-4 border-t border-neutral-100 dark:border-white/[0.06] flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl bg-neutral-900 hover:bg-black dark:bg-white dark:hover:bg-neutral-100 text-white dark:text-neutral-900 font-semibold text-[14px] flex items-center justify-center gap-2 transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer active:scale-[0.98]"
                >
                  {loading ? (
