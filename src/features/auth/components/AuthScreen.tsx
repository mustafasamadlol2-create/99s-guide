...
async function oauthResponseError(response: Response, fallback: string): Promise<Error & { status?: number }> {
  const body = await response.json().catch(() => ({})) as { error?: string };
  const error = Object.assign(new Error(body.error || fallback), { status: response.status });
  return error;
}
...
interface AuthScreenProps {
  ...
  }) => Promise<void | { verificationRequired?: boolean; initialEmailDeliveryFailed?: boolean }>;
}
...
export default function AuthScreen({ onNavigateToLegal, onLoginSuccess }: AuthScreenProps) {
  ...
  const [registrationVerificationRequired, setRegistrationVerificationRequired] = useState(false);
  const [initialEmailDeliveryFailed, setInitialEmailDeliveryFailed] = useState(false);
  ...
  const [resendVerificationState, setResendVerificationState] = useState<"idle" | "loading" | "sent" | "error">("idle");
  ...
  const browserFinishedTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);  // grace timer while callback exchange completes
  ...
  const go = useCallback((next: AuthMode) => {
    ...
    setMode(next);
    setError("");
    setRegistrationVerificationRequired(false);
    setInitialEmailDeliveryFailed(false);
  }, []);
  ...
  // ── Handle email verification redirect errors ──────────────────────────────
  // When the verification link is expired or invalid, App.tsx dispatches this
  // custom event so AuthScreen can show a clear message with a resend option.
  useEffect(() => {
    const handleVerificationError = (e: Event) => {
      const reason = (e as CustomEvent<string>).detail || "invalid";
      const friendly =
        reason === "expired"
          ? "This verification link has expired. Please request a new one."
          : "This verification link is invalid. Please request a new one.";
      showError(friendly);
      // Switch to login mode so the user can navigate to resend flow.
      setMode("login");
    };
    window.addEventListener("verification-redirect-error", handleVerificationError);
    return () => window.removeEventListener("verification-redirect-error", handleVerificationError);
  }, [showError]);
  ...
  useEffect(() => {
    const handleOAuthMessage = (event: MessageEvent) => {
      ...
      // Same-window synthetic events have no source. Any cross-window auth
      // message must come from the popup opened by this specific attempt.
      if (event.source && event.source !== oauthPopupRef.current) return;
      ...
      if (type === "OAUTH_AUTH_SUCCESS") {
        // A real popup message is handled by the popup-specific listener below,
        // which validates its WindowProxy and relays a synthetic same-window
        // event. Consuming it here first would set oauthCompletedRef and make
        // the popup handler drop the successful Apple/Google result.
        ...
        // Stop any running native poll as well — the session was delivered.
        if (oauthPollRef.current !== null) {
          clearInterval(oauthPollRef.current);
          oauthPollRef.current = null;
        }
        ...
        // Unblock future attempts. The success path bypasses the popup-specific
        // onPopupSuccess handler (it early-returns once oauthCompletedRef is
        // set above), so without this reset oauthInFlightRef would stay true
        // and every subsequent sign-in tap would be silently ignored.
        oauthInFlightRef.current = false;
    ...
    if (mode === "forgot") {
      ...
      try {
        const res  = await apiClient("/api/auth/forgot-password", {
          ...
          // Failures are surfaced inline via showError below — never duplicate
          // them as a global toast.
          silent:  true,
        });
  ...
  const handleResendVerification = async () => {
    if (resendVerificationState === "loading") return;
    setResendVerificationState("loading");
    setError("");
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setResendVerificationState("sent");
      } else {
        setResendVerificationState("error");
        showError(data.error || "Failed to resend the verification email.");
      }
    } catch (err: any) {
      setResendVerificationState("error");
      showError(err.message || "Connection error resending the verification email.");
    }
  };
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (registerStep === 4) {
      ...
      try {
        ...
        const authResult = await onLoginSuccess({
          name: name.trim(),
          email: email.trim(),
          password,
          studentGroup,
        ...
        setRegistrationVerificationRequired(
          !!authResult && typeof authResult === "object" && authResult.verificationRequired === true,
        );
        setInitialEmailDeliveryFailed(
          !!authResult && typeof authResult === "object" && !!(authResult as any).initialEmailDeliveryFailed,
        );
        setRegisterStep(5);
      } catch (err: any) {
        setRegisterStep(4);
    ...
    if (NativeBridge.isNativePlatform()) {
      ...
      try {
        const pkce = await createPkcePair();
         const res = await apiClient(`/api/auth/oauth-url?provider=${key}&flow=inapp&code_challenge=${encodeURIComponent(pkce.challenge)}`, { silent: true });
           if (!res.ok) throw await oauthResponseError(res, `Failed to initialize ${provider} login session.`);
        const data = await res.json();
        ...
        if (stateToken) {
          await SecureStorage.set(`oauth_pkce_${stateToken}`, pkce.verifier);
        ...
        Browser.addListener("browserFinished", () => {
          ...
          // The provider callback can close the browser before the server has
          // finished storing the one-time PKCE session. Keep the button in its
          // loading state while the poll completes; only a browser close that
          // remains unresolved is treated as cancellation. The 45 s window
          // covers slow callback exchanges (e.g. backend cold starts) so a
          // completing sign-in is never misreported as cancelled.
            ...
            try {
              ...
              const pollRes = await apiClient(`/api/auth/oauth-session/${stateToken}`, {
                ...
                headers: {
                  ...
                  "X-OAuth-State": stateToken,
                },
                body: JSON.stringify({ code_verifier: pkce.verifier }),
                // A pending OAuth callback is an expected intermediate state.
                // Do not turn its 404 { pending: true } response into a global
                // error toast while the provider flow is still open.
                silent: true,
              });
              if (pollRes.ok) {
                ...
                if (pollData.success && pollData.token) {
                  ...
                  oauthInFlightRef.current = false;
                  await SecureStorage.remove(`oauth_pkce_${stateToken}`);
                  ...
                  localStorage.removeItem("_oauth_pending_token");
                  Browser.close().catch(() => {});
                  onOAuthFailed(pollData.message);
                }
              }
            } catch (err: any) {
      ...
      // ── Web path ────────────────────────────────────────────────────────────
      //
      // Strategy: Try popup flow for ALL web platforms (including iOS Safari).
      // iOS Safari opens a new tab via window.open(), but that tab:
      //   - CAN be closed by window.close() (it was opened by script)
      //   - Preserves window.opener for postMessage
      //   - Receives token via Bearer header, avoiding cookie partitioning
      // If popup is blocked, we fall back to redirect flow below.
      //
      ...
      // issue entirely.
        ...
        if (!popup) {
          // Popup was blocked — use the PKCE redirect flow as fallback
          // instead of treating a browser policy decision as auth failure.
          try {
            const pkce = await createPkcePair();
            const res = await apiClient(
              `/api/auth/oauth-url?provider=${key}&flow=redirect&code_challenge=${encodeURIComponent(pkce.challenge)}`,
              { silent: true },
            );
           if (!res.ok) throw await oauthResponseError(res, `Failed to initialize ${provider} login session.`);
            const data = await res.json();
        ...
        const cleanupPopupListeners = () => {
          ...
          localStorage.removeItem("_oauth_pending_token");
          if (stateToken) void SecureStorage.remove(`oauth_pkce_${stateToken}`);
        };
        ...
        try {
          ...
          codeVerifier = pkce.verifier;
           const res = await apiClient(`/api/auth/oauth-url?provider=${key}&code_challenge=${encodeURIComponent(pkce.challenge)}`, { silent: true });
           if (!res.ok) throw await oauthResponseError(res, `Failed to initialize ${provider} login session.`);
          const data = await res.json();
          const targetUrl = data.url ?? data.sandboxUrl;
          if (!targetUrl) throw new Error(`Authentication URL was not returned for ${provider}.`);
        ...
        // The callback page is served by the Render API, so accept messages from
        // that configured API origin as well as the frontend origin.
        ...
        // ── Step 4 — session-driven completion poller ─────────────────────────
        // postMessage (Step 3) is the fast path. This poller is the
        // authoritative fallback and the ONLY place a sign-in outcome is
        // concluded. It never derives a verdict from the popup window state
        // alone: on several browsers the popup's WindowProxy is unreliable
        // while the Google chooser is open (cross-origin SecurityErrors, or a
        // stale reference reporting `closed === true` even though the chooser
        // is still on screen). A "closed-looking" popup does NOT mean the
        // sign-in failed, so the window state only starts a generous grace
        // countdown — the real verdict comes from the server session, which
        // always wins when it resolves.
        let popupClosedTicks = 0;
        let pollInFlight = false;
        const startedAt = Date.now();
        const POPUP_TICK_MS = 1500;
        const CLOSE_GRACE_MS = 30000;      // sustained closed + no session → cancelled
        const MAX_WAIT_MS = 5 * 60 * 1000; // hard ceiling → timed out

        popupClosedIntervalRef.current = setInterval(async () => {
          if (oauthCompletedRef.current) {
            cleanupPopupListeners(); // already handled by postMessage
            return;
          }

          // Hard ceiling — the flow can never spin forever.
          if (Date.now() - startedAt > MAX_WAIT_MS) {
            onPopupFailed("Sign-in timed out. Please try again.");
            return;
          }

          // Weak signal only. An unreadable or "closed" popup never decides
          // anything by itself — it only starts the cancellation countdown.
          let isClosed = false;
          try { isClosed = popup.closed; } catch { isClosed = false; }
          popupClosedTicks = isClosed ? popupClosedTicks + 1 : 0;

          // Poll the session — the only trustworthy source of truth. It runs
          // while the popup is open too, so a completing sign-in is detected
          // even when postMessage is blocked (Safari ITP / partitioned popup
          // jars) or the popup reference is broken.
          if (stateToken) {
            if (pollInFlight) return;
            pollInFlight = true;
            try {
              const storedVerifier = codeVerifier || await SecureStorage.get(`oauth_pkce_${stateToken}`);
              const pollRes = await apiClient(`/api/auth/oauth-session/${stateToken}`, {
                // Pending/cancelled states are classified locally; they must
                // not trigger the app-wide API error toast.
                method: "POST",
                silent: true,
                headers: { "Content-Type": "application/json", "X-OAuth-State": stateToken },
                body: JSON.stringify({ code_verifier: storedVerifier ?? "" }),
              });
              if (pollRes.ok) {
                const pollData = await pollRes.json() as {
                  success?: boolean; token?: string; userId?: string; email?: string;
                  rejected?: boolean; pending?: boolean; message?: string;
                };
                if (pollData.success) {
                ...
                if (pollData.rejected) {
                  onDomainRejected(pollData.message);
                  return;
                }
              }
              // 404 { pending } / 429 / other non-ok → inconclusive; keep waiting.
            } catch (err: any) {
          ...
          // Only a SUSTAINED "popup closed + no session" is a genuine manual
          // cancellation — long enough for any completing callback to land.
          if (popupClosedTicks * POPUP_TICK_MS >= CLOSE_GRACE_MS) {
            onPopupFailed("Sign-in was cancelled. Please try again.");
          }
        }, POPUP_TICK_MS);
    }
                    ...
                    >
                          ...
                          <div className="space-y-1.5 text-left">
                            <label className="text-[13px] ml-1 font-semibold text-neutral-500 dark:text-[#EBEBF599] uppercase tracking-wider flex justify-between items-center">
                              <span className="flex items-center gap-1.5">
                                Signature
                                <span className="text-[10px] font-normal normal-case tracking-normal text-neutral-400 dark:text-white/30">(Optional)</span>
                              </span>
                              {signature && !isSigning && (
                                <button
                                  type="button"
                                  onClick={() => setIsSigning(true)}
                                  className="text-blue-500 text-xs normal-case"
                                >
                                  Redraw
                                </button>
                              )}
                            ...
                            {isSigning || !signature ? (
                              ...
                              <div
                                ...
                                onClick={() => setIsSigning(true)}
                              >
                                <img src={signature} alt="Your signature" className="max-h-full max-w-full object-contain dark:invert" />
                              </div>
                            )}
                            ...
                            {signature && !isSigning && (
                              <button
                                type="button"
                                onClick={() => { setSignature(null); setIsSigning(false); }}
                                className="text-[12px] text-neutral-400 dark:text-white/30 hover:text-red-400 dark:hover:text-red-400 transition-colors ml-1"
                              >
                                Clear signature
                              </button>
                            )}
                      ...
                      {registerStep === 5 && (
                        <div
                          ...
                          role="status"
                          aria-live="polite"
                           aria-label={isLoading ? "Creating your account" : registrationVerificationRequired ? "Email verification required" : "Registration complete"}
                          className="text-center py-6 focus:outline-none"
                        >
                          ...
                          >
                            ...
                            >
                               {isLoading
                                 ...
                                 : registrationVerificationRequired
                                   ? initialEmailDeliveryFailed
                                     ? `We couldn't send a verification email to ${email}. Tap below to resend.`
                                     : `A verification link was sent to ${email}. Verify the address before signing in.`
                          ...
                          {registrationVerificationRequired && !isLoading && (
                            <motion.div
                              initial={{ opacity: 0 }}
                              animate={{ opacity: 1 }}
                            ...
                            >
                              <button
                                type="button"
                                disabled={resendVerificationState === "loading" || resendVerificationState === "sent"}
                                onClick={handleResendVerification}
                                aria-live="polite"
                                className="text-caption font-semibold text-med-blue dark:text-amber-400 hover:underline disabled:opacity-50 disabled:no-underline disabled:cursor-not-allowed cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-med-blue rounded-sm px-1 py-0.5"
                              >
                                {resendVerificationState === "loading"
                                  ? "Sending…"
                                  : resendVerificationState === "sent"
              ...
              {mode === "login" && (
                ...
                >
                  ...
                  >
                    ...
                    {/* Social divider — hidden in social-only authentication mode */}
                    {!socialOnlyAuth && <motion.div variants={FIELD_V as any} className="relative my-2 text-center">
