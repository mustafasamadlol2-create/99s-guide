...
const MonthDayCell = memo(function MonthDayCell({
 ...
 <div
 key={`day-${day}`}
  className={`group relative min-h-[64px] md:min-h-[86px] md:aspect-square w-full flex flex-col justify-start items-stretch text-left transition duration-200 select-none ${showPopover ? "z-50" : "z-0"}
 ...
 {/* Day Header with numbers and indicator dots */}
        <div className="flex justify-between items-start w-full shrink-0 mb-0.5 px-1 pt-1">
          ...
          >
           ...
           {dayEvents.length > 0 && (
             <span className="calendar-event-count text-[9px] font-medium text-neutral-500 dark:text-[#EBEBF599] bg-black/5 dark:bg-white/[0.12] px-1.5 py-0.5 rounded-full mt-0.5 whitespace-nowrap">
