...
export const CalendarWeekView = memo(function CalendarWeekView({
 ...
 /* Direction-aware touch handling: only block propagation for horizontal swipes
    so vertical scrolling still reaches the parent canvas on mobile. */
 ...
 const handleTouchMove = useCallback((e: React.TouchEvent) => {
   ...
   /* Horizontal swipe threshold: block parent vertical scroll */
   if (dx > dy && dx > 8) {
     e.stopPropagation();
   }
 }, []);
 ...
 const handleTouchEnd = useCallback(() => {
  ...
  return (
  <div id="calendar_week_grid_deck" tabIndex={0} role="application" aria-label="Interactive weekly timeline" className="space-y-4 md:space-y-4">
  <div
  id="week_columns_container"
  
  className="flex overflow-x-auto touch-pan-x snap-x snap-mandatory md:grid md:grid-cols-5 gap-3 md:gap-2 pb-4 scroll-smooth hide-scrollbar px-4 md:px-0 scroll-px-4 md:scroll-px-0"
  style={{
