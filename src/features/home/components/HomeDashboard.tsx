...
interface PremiumMottoTransitionProps {
  text: string;
  isRtl: boolean;
...
/**
 * A single-raster motto transition for iOS/WKWebView.
 *
 * The text node is never duplicated. We fade the current glyph raster fully to
 * zero, swap the string while the SAME composited layer is invisible, wait for
 * two browser paints, then run the approved premium entrance. This removes the
 * WebKit stale-layer overlap/ghosting that can happen when two keyed text nodes
 * are mounted/unmounted around the same frame.
 */
const PremiumMottoTransition = memo(({
  text,
  isRtl,
  reduceMotion,
  subtitleClass,
}: PremiumMottoTransitionProps) => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // 1) Remove the old glyph raster completely. No translation here: moving
        // text while fading is what made Safari expose stale edge pixels.
        ...
        // 2) Swap content only while the single layer is fully transparent.
        displayedTextRef.current = text;
        setDisplayedText(text);
        controls.set({
          opacity: 0,
        ...
        // Two paints make WebKit discard the previous text raster before the
        // new layer becomes visible. This is the no-overlap guarantee.
        await waitForCleanPaint();
        if (cancelled || sequence !== sequenceRef.current) return;

        // Restart only the decorative sheen for the incoming message.
        setSheenKey((value) => value + 1);

        // 3) Preserve the approved premium entrance from the previous version.
        await controls.start({
          opacity: 1,
          y: 0,
          x: 0,
          scale: 1,
          transition: {
            opacity: { duration: 0.46, ease: [0.16, 1, 0.3, 1] },
            y: { duration: 0.76, ease: [0.16, 1, 0.3, 1] },
            x: { duration: 0.68, ease: [0.16, 1, 0.3, 1] },
            scale: { duration: 0.78, ease: [0.16, 1, 0.3, 1] },
          },
        });
      } catch {
        // Stopping an in-flight Motion animation during a rapid data refresh is
        // safe; the next sequence owns the layer and will paint the latest text.
      }
    };
  ...
  }, [text, isRtl, reduceMotion, controls]);
    ...
    >
      {!reduceMotion && (
        <motion.span
          ...
          initial={{ opacity: 0, x: isRtl ? "112%" : "-112%", scaleX: 0.78 }}
          animate={{
            opacity: [0, 0.46, 0],
            x: isRtl ? ["112%", "8%", "-118%"] : ["-112%", "8%", "118%"],
            scaleX: [0.78, 1, 0.88],
          }}
          transition={{
            duration: 0.92,
      ...
      <p
        className={`${subtitleClass} home-motto-text relative z-[1] text-white/90 font-medium leading-snug md:leading-normal break-words antialiased w-full`}
      >
        {displayedText}
      </p>
    </motion.div>
...
PremiumMottoTransition.displayName = "PremiumMottoTransition";
...
const HeroBanner = memo(({
  ...
  // Phone portrait hero layout is deliberately platform-neutral.
  // PWA and Capacitor now render the exact same hero geometry; no native-only
  // scale/height compensation is applied here. This keeps every approved
  // badge, greeting, motto, and identity-pill position identical across both.
          ...
          <div className="flex flex-col relative z-10">
            ...
            >
              ...
              >
                ...
                {/* Single-layer iOS-safe transition: the old and new motto can never
                    be visible together, while the approved entrance motion stays intact. */}
                <div
                  className="home-motto-stage relative flex items-start overflow-hidden"
                  style={{ minHeight: "2.8rem", isolation: "isolate" }}
                >
                  <PremiumMottoTransition
                    text={smartSubtitle.text}
                    isRtl={isRtl}
  ...
  const layout = useMemo(() => {
    ...
    if (h === "compact" && v === "compact") {
      ...
      return {
        ...
        titleClass: "personalization-reading-home-title font-semibold ",
      ...
      if (width < 800) {
        ...
        return {
          ...
          brandClass: "text-sm tracking-[0.36em]",
          titleClass:
            "personalization-reading-home-title font-semibold leading-[1.05] origin-left scale-100",
