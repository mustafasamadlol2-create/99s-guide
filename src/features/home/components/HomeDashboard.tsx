...
const StarField = ({ opacity = "opacity-[0.35]" }: { opacity?: string }) => {
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      if (entries.length > 0) {
        ...
        // Re-draw static frame on resize when motion is reduced
    ...
    const render = () => {
      ...
      stars.forEach((star) => {
        ...
        star.xPct = (star.xPct + star.speed * 0.5) % 1.0;
        star.yPct = (star.yPct - star.speed * 0.3 + 1.0) % 1.0;
      });
...
const AmbientGlow = ({ isRtl }: { isRtl: boolean }) => (
  <>
    ...
    <div className="absolute bottom-0 left-0 w-[55%] h-[65%] bg-[radial-gradient(ellipse_at_bottom_left,_rgba(212,175,55,0.07)_0%,_transparent_68%)] pointer-events-none z-0" />
    {/* Faint blue-violet accent — top-right depth */}
    <div className="absolute top-0 right-0 w-[45%] h-[55%] bg-[radial-gradient(ellipse_at_top_right,_rgba(56,100,210,0.08)_0%,_transparent_65%)] pointer-events-none z-0" />
  </>
);
...
interface HeroBannerProps {
  ...
  user: { id?: string; name: string; email: string; totalPoints: number; level: string; levelBadge: string; streakDays: number };
...
const UpcomingEventAlert = memo(({ nextEvent, isRtl, onNavigateTab, t }: { nextEvent: CalendarEvent; isRtl: boolean; onNavigateTab: (tab: string) => void; t: any }) => {
  ...
  return (
    <div
      className={`p-3.5 rounded-lg ${bgClass} border border-black/5 dark:border-white/[0.12] flex items-center justify-between gap-2.5 ${colorClass} shadow-elevation-1 dark:shadow-elevation-0 transition duration-300`}
    ...
    >
      ...
      <button
        ...
        className="text-xs text-[#804D00] bg-white hover:bg-[#FFF5E5] rounded-lg border border-[#FFD699] dark:bg-[#1C1C1E] dark:text-[var(--text-primary)]/90 dark:border-white/[0.12] dark:hover:bg-white/[0.12] shadow-elevation-1 shrink-0 px-4 py-2 font-semibold cursor-pointer transition-colors outline-none focus-visible:ring-2 focus-visible:ring-med-blue focus-visible:ring-offset-2 dark:focus-visible:ring-offset-neutral-950 antialiased"
      >
        {t("openSchedule")}
      </button>
    </div>
...
const HeroBanner = memo(({
  isRtl, smartGreeting, user, smartSubtitle, activeMottos,
  mottoIndex, layout, t,
  ...
  return (
    <div
      className={[
        "relative rounded-xl md:rounded-xl bg-[#05070B] text-white",
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden rounded-xl pointer-events-none z-0">
        ...
        <div className="absolute inset-0 z-0">
          ...
          <StarField opacity="opacity-[0.48]" />
        </div>
        ...
        <div className={`flex flex-col min-w-0 overflow-hidden ${isWide ? "flex-1" : ""} ${layout.gapClass}`}>
          ...
          <div className="flex flex-col relative z-10">
            ...
            <motion.div
              ...
              className={`${layout.stackGapClass} ${isWide ? "max-w-[85%]" : "max-w-[80%] md:max-w-[72%]"} min-w-[260px] relative mt-1 md:mt-2`}
            >
              <div
          ...
          <ul role="list" aria-label="Student identity" className="flex flex-wrap items-center gap-2.5 md:gap-3 pt-3 relative z-10 list-none p-0 m-0">
...
const HomeDashboard = memo(function HomeDashboard({
  ...
  useEffect(() => {
    if (activeMottos.length <= 1) return;
  ...
  const { pullY, isRefreshing, containerRef } = usePullToRefresh({
    ...
    const dbLecturesBySubject = new Map<string, any[]>();
    if (dbLectures) {
      for (const l of dbLectures) {
        if (!dbLecturesBySubject.has(l.mainSubject)) {
          dbLecturesBySubject.set(l.mainSubject, []);
        }
        const list = dbLecturesBySubject.get(l.mainSubject); if (list) list.push(l);
      }
    }

    subjects.forEach((subject) => {
      const subjectLectures = subject.modules.flatMap((m) => m.lectures);
      const subjectDbLectures = dbLecturesBySubject.get(subject.id) || [];
      const totalTasks = (subjectLectures.length + subjectDbLectures.length) * 5;
      ...
      subjectDbLectures.forEach(countTasks);
  ...
  // Performance Optimization: Precompute and memoize subject lecture counts in O(N + M) time
  const subjectLectureCounts = useMemo(() => {
    const dbCountsBySubject: Record<string, number> = {};
    if (dbLectures) {
      for (const l of dbLectures) {
        dbCountsBySubject[l.mainSubject] = (dbCountsBySubject[l.mainSubject] || 0) + 1;
      }
    }

    return subjects.reduce(
      (acc, subject) => {
        const seededCount = subject.modules.reduce(
          (sum, m) => sum + m.lectures.length,
          0,
        );
        acc[subject.id] = seededCount + (dbCountsBySubject[subject.id] || 0);
        return acc;
      },
  ...
  const layout = useMemo(() => {
    ...
    if (h === "compact" && v === "compact") {
      ...
      return {
        ...
        pillsClass: "px-4 py-2 text-xs rounded-full",
        gapClass: "space-y-4",
        ...
        <div className={`w-full flex flex-col ${device.spacing} items-start`}>
          ...
          <div className={`w-full flex flex-col ${device.spacing}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-start">
              <div className="w-full relative z-30 ios-staggered-card">
                <CommandPalette
                  isOpen={true}
                  onClose={() => {}}
                  inline={true}
              ...
              <button
                className={`
 ios-staggered-card macos-interactive relative group flex items-center gap-3 p-4
