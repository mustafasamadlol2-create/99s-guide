...
const VideoCard = ({ video, videoId, onWatch }: { video: any, videoId: string | undefined, onWatch: (url: string) => void }) => {
  const [realDuration, setRealDuration] = useState<number | null>(null);

  useEffect(() => {
    if (!videoId) return;
    const div = document.createElement("div");
    div.id = `yt-player-${videoId}-${Math.random().toString(36).substr(2, 9)}`;
        ...
        events: {
          onReady: (event: any) => {
            ...
            if (time > 0) {
              ...
              setTimeout(() => {
                const t2 = event.target.getDuration();
                if (t2 > 0) setRealDuration(t2);
              }, 1000);
            }
            setTimeout(() => {
  ...
  return (
    <div
      ...
      className="group flex flex-row items-center bg-white dark:bg-[#1C1C1E] border border-neutral-200/80 dark:border-white/[0.12] rounded-xl shadow-sm hover:shadow-md transition p-3 gap-4 cursor-pointer"
    >
      <div className="relative w-32 shrink-0 aspect-video bg-neutral-100 dark:bg-neutral-800 rounded-lg overflow-hidden">
        {videoId ? (
          <img
            loading="lazy"
            decoding="async"
        ...
        {realDuration && (
           <div className="absolute bottom-1 right-1 bg-black/80 backdrop-blur-sm text-white text-[10px] font-mono px-1.5 py-0.5 rounded-md font-medium">
      ...
      <div className="flex-1 min-w-0 py-1 text-left">
        <h4 className="text-sm font-semibold text-neutral-900 dark:text-white group-hover:text-red-600 transition leading-snug">
          {video.title}
        </h4>
        {video.description && (
        ...
        <button
          className="inline-flex items-center justify-center px-4 py-1.5 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white text-xs font-semibold rounded-full transition cursor-pointer pointer-events-none"
        >
...
export const LectureDetailView = function LectureDetailView({
 ...
 const baseList =
 detailedLecture.isDatabaseLecture && detailedLecture.flashcards
 ...
 }, [lecture, customFlashcards]);
 const getRelevantCards = useCallback(() => relevantCards, [relevantCards]);

 const relevantVideos = useMemo(() => {
 if (lecture.isDatabaseLecture && lecture.materials) {
 const dbVideos = lecture.materials.filter((m: any) => m.type === "VIDEO");
 ...
 if (detailedLecture.isDatabaseLecture && detailedLecture.mcqs && detailedLecture.mcqs.length > 0) {
 ...
 // Video watching — opens in external browser/YouTube app
 const handleWatchVideo = (youtubeUrl: string) => {
   NativeBridge.openUrl(youtubeUrl, "_blank");

   if (!videoWatched) {
     setVideoWatched(true);
     onUpdateProgress({ videoCompleted: true });
     onAddPoints(3, `Watched medical video tutorial for ${lecture.title}`);
   }
 };
...
const handleDeleteAnswer = async (qId: string, ansId: string) => {
 ...
 {/* 1. Header Toolbar */}
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/80 dark:bg-[#1C1C1E]/80 backdrop-blur-sm p-4 sm:p-5 border border-black/[0.04] dark:border-white/[0.06] rounded-xl shadow-elevation-1 dark:shadow-[0_2px_10px_rgba(0,0,0,0.4)]">
 ...
 className="flex items-center gap-1 text-med-blue hover:text-med-blue dark:text-blue-400 font-medium px-2 py-2 rounded-lg transition-colors cursor-pointer/40 shrink-0"
 ...
 <h1 className="text-lg sm:text-2xl leading-[1.2] font-display font-semibold text-neutral-900 dark:text-[var(--text-primary)] truncate mt-1 antialiased">
 {lecture.title}
 </h1>
 ...
 <div className="relative bg-black/[0.04] dark:bg-white/[0.06] p-1 rounded-lg flex items-center select-none h-8 w-full sm:w-auto sm:min-w-[420px] shrink-0 antialiased">
 {[
 { id: "pdf", label: "PDF" },
 ...
 <h2 className="text-headline font-display font-semibold text-neutral-800 dark:text-[var(--text-primary)]">
 {lecture.title}
 </h2>
 </div>
 ...
 type="button"
 onClick={async () => {
                        let freshPdfUrl = lecture.pdfUrl;
                        try {
                         const parsed = new URL(freshPdfUrl);
                         if (parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1") {
                          freshPdfUrl = parsed.pathname + parsed.search + parsed.hash;
                         }
                        } catch(e) {}

                        const baseLink =
                          freshPdfUrl.startsWith("http") ||
                          freshPdfUrl.startsWith("/uploads/") || freshPdfUrl.startsWith("/api/materials/")
                            ? freshPdfUrl
                            : `/assets/${freshPdfUrl}`;
                        let cleanLink = baseLink;
                        
                        if (cleanLink.startsWith("/")) {
                          // On Capacitor native, window.location.origin is "capacitor://localhost".
                          // Use the API server origin so /uploads and /api paths resolve correctly.
                          cleanLink = (getApiBaseUrl() || window.location.origin) + cleanLink;
                        }

                        NativeBridge.openUrl(cleanLink, "_blank");
 ...
 className="w-full sm:w-auto px-8 py-4 bg-med-blue hover:bg-med-blue text-white rounded-lg text-base font-semibold flex items-center justify-center gap-3 cursor-pointer shadow-elevation-1 hover:shadow-elevation-1 antialiased"
 >
 ...
 type="button"
 onClick={async () => {
                        let freshNotesUrl = lecture.notesPdfUrl || "";
                        try {
                         const parsed = new URL(freshNotesUrl);
                         if (parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1") {
                          freshNotesUrl = parsed.pathname + parsed.search + parsed.hash;
                         }
                        } catch(e) {}

                        const baseLink =
                          freshNotesUrl?.startsWith("http") ||
                          freshNotesUrl?.startsWith("/uploads/") || freshNotesUrl?.startsWith("/api/materials/")
                            ? freshNotesUrl
                            : `/assets/${freshNotesUrl}`;
                        let cleanLink = baseLink || "#";
                        ...
                        // Open notes in external browser
                        NativeBridge.openUrl(cleanLink, "_blank");
 ...
 className="w-full sm:w-auto px-8 py-4 bg-purple-500 hover:bg-purple-600 text-white rounded-lg text-base font-semibold flex items-center justify-center gap-3 cursor-pointer shadow-elevation-1 hover:shadow-elevation-1 antialiased"
 >
 <ExternalLink className="w-icon-sm h-icon-sm" />{" "}
 {isRtl
 ...
 className={`w-full p-4 text-left text-caption rounded-lg border transition flex items-center gap-3 cursor-pointer ${
 isSelected
 ...
 className="h-btn px-6 py-2 text-caption font-semibold rounded-md border border-neutral-200 dark:border-white/[0.12] text-neutral-600 dark:text-[var(--text-secondary)] hover:bg-neutral-50 dark:hover:bg-white/[0.12] disabled:opacity-50 cursor-pointer transition-colors"
 >
 ...
 className={`h-btn px-6 py-2 text-caption font-semibold rounded-md border cursor-pointer flex items-center gap-2 transition ${
 showHint
 ? "bg-[#FF9500]/15 border-[#FF9500]/30 text-[#FF9500]"
 : "bg-[#FF9500]/10 hover:bg-[#FF9500]/15 border-[#FF9500]/25 text-[#FF9500]"
 }`}
 >
 <HelpCircle className="w-icon-sm h-icon-sm text-[#FF9500]" />
 {showHint
 ? isRtl
 ...
 className="px-5 py-2 text-caption bg-[#007AFF] hover:bg-[#007AFF]/90 text-white font-semibold rounded-full shadow-elevation-1 min-w-[124px] cursor-pointer transition duration-155"
 >
 ...
 <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-neutral-100 dark:border-white/[0.06] pb-3 mb-3">
