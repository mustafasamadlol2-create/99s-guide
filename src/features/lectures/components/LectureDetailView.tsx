...
const VideoCard = ({
  ...
  useEffect(() => {
    ...
    div.id = `yt-player-${videoId}-${Math.random()
      .toString(36)
      .substring(2, 11)}`;
    ...
    const loadPlayer = () => {
      ...
      new (window as any).YT.Player(div.id, {
        height: "0",
        width: "0",
        videoId,
        playerVars: {
  ...
  const handleVideoPress = () => {
    ...
    console.log("[YT UI] VideoCard pressed", {
      url,
      videoId,
      title: video?.title,
    });
    ...
    if (!url) {
      console.error("[YT UI] Missing YouTube URL", video);
      return;
    }

    void Promise.resolve(onWatch(url)).catch((error) => {
      console.error("[YT UI] onWatch failed", error);
    });
  };
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 min-w-0 py-1 text-left">
        <span className="block text-sm font-semibold text-neutral-900 dark:text-white group-hover:text-red-600 transition leading-snug">
          {video?.title}
        </span>
...
export const LectureDetailView = function LectureDetailView({
 ...
 const extractYouTubeId = (url: string): string | null => {
  ...
  // Video watching — opens the native YouTube app on iOS, web target elsewhere
  const handleWatchVideo = async (youtubeUrl: string) => {
    ...
    try {
      if (isNativeIOS) {
        ...
        const result = await NativeBridge.openYouTubeUrl(cleanUrl);

        console.log("[YT TAP] NativeBridge result =", result);
      } else {
        console.log("[YT TAP] Web path selected");

        if (popupWindow) popupWindow.location.href = cleanUrl; else await NativeBridge.openUrl(cleanUrl, "_blank");
      }
    } catch (error) {
      if (popupWindow) popupWindow.close(); youtubeExternalOpenRef.current = false;
      ...
      showiOSAlert({
        ...
        message:
          "The YouTube application could not be opened. Please try again.",
        actions: [{ label: "OK", style: "default" }],
      });
...
const handleDeleteAnswer = async (qId: string, ansId: string) => {
 ...
 <div className="absolute right-0 top-[20%] bottom-[20%] w-0 bg-black/[0.1] dark:bg-white/[0.1] z-0" />
  ...
  <motion.div
    ...
    className="bg-white dark:bg-[#1C1C1E] border border-med-beige/60 dark:border-neutral-850 rounded-lg shadow-elevation-1 min-h-[200px] flex flex-col relative"
  >
