/**
 * BannedUsersView — admin panel listing all currently banned users.
 * Supports: Remove Ban, Change Duration, Convert to Permanent.
 */
import React, { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "motion/react";
import { ShieldOff, Loader2, AlertCircle, CheckCheck, RefreshCw, ChevronDown, Infinity as InfinityIcon, Clock } from "lucide-react";
...
export const BannedUsersView: React.FC = () => {
  ...
  // Re-fetch whenever the server broadcasts a ban list change
  useEffect(() => {
    const handler = () => fetchBans();
    window.addEventListener("socket-ban-list-updated", handler);
    return () => window.removeEventListener("socket-ban-list-updated", handler);
  }, [fetchBans]);

  const handleRemove = useCallback(async (userId: string) => {
    setActionLoading(userId + ":remove");
    try {
      const res = await apiClient(`/api/moderation/bans/${userId}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
      setUsers(prev => prev.filter(u => u.userId !== userId));
      showFeedback("✅ Ban lifted. Account access restored.");
    } catch {
      showFeedback("❌ Failed to remove ban.");
    } finally {
  ...
  const handleMakePermanent = useCallback(async (userId: string) => {
    ...
    try {
      const res = await apiClient(`/api/moderation/bans/${userId}`, {
        method: "PATCH",
      ...
      showFeedback("✅ Ban converted to permanent.");
    } catch {
  ...
  const handleUpdateDuration = useCallback(async (userId: string) => {
    ...
    try {
      ...
      setEditingId(null);
      showFeedback("✅ Ban duration updated.");
    } catch {
    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="border-b border-neutral-100 dark:border-white/[0.12] pb-3 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <ShieldOff className="w-4 h-4 text-neutral-600 dark:text-neutral-400" />
          <h3 className="text-headline font-display font-semibold text-neutral-800 dark:text-white">
            Banned Users
          </h3>
        </div>
        <button onClick={fetchBans} className="p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-white/[0.08] text-neutral-500 transition-colors cursor-pointer" title="Refresh">
      ...
      {!loading && !error && users.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
          ...
          <p className="text-[15px] font-semibold text-neutral-700 dark:text-white">No banned users</p>
          <p className="text-[13px] text-neutral-500">No accounts are currently suspended.</p>
        </div>
      )}
            ...
            return (
              <div key={user.userId} className="bg-white dark:bg-[#1C1C1E] border border-neutral-200/50 dark:border-white/[0.08] rounded-xl p-4 space-y-3 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                  <UserAvatar name={user.name} avatarUrl={user.avatar} className="w-10 h-10 rounded-full border border-neutral-200 dark:border-white/[0.12] shrink-0 opacity-60" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-[14px] font-semibold text-neutral-900 dark:text-white truncate">{user.name}</p>
                      {user.isPermanent ? (
                        <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-neutral-200 text-neutral-700 dark:bg-white/[0.10] dark:text-neutral-300">
                  ...
                  <div className="flex items-center gap-1 text-[12px] text-rose-600 dark:text-rose-400 font-medium shrink-0">
                ...
                <p className="text-[12px] text-neutral-400">
                  Banned since {new Date(user.startTime).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}
                </p>
