...
// xlsx is loaded on demand — only imported when the admin clicks "Excel export"
      ...
      {record && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 z-50 w-full max-w-lg bg-white dark:bg-[#1C1C1E] border-l border-neutral-200 dark:border-white/[0.08] shadow-2xl overflow-y-auto"
          >
            {/* Header */}
...
const ModerationHistoryView: React.FC = () => {
  ...
  const exportExcel = async () => {
    try {
      // Dynamically import xlsx only when the admin actually clicks Export — keeps
      // the chunk lean for the common case where export is never used in a session.
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Moderation History");
      XLSX.writeFile(wb, `moderation-history-${new Date().toISOString().slice(0,10)}.xlsx`);
    } catch { /* silent */ }
  };
