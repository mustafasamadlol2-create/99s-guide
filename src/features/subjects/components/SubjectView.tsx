...
export const SubjectView = function SubjectView({
  ...
  const getDefaultSubSubject = (subjId: string, isRtl: boolean) => {
    if (subjId === "ID") return null;
    const items: Record<string, { en: string; ar: string }> = {
      ID: { en: "Infectious Diseases", ar: "الأمراض الانتقالية وعلم الأحياء" },
      NT: { en: "Clinical Nutrition", ar: "التغذية السريرية" },
      RM: { en: "Research Methodology & Biostatistics", ar: "مناهج البحث والتحليل" },
      CA: { en: "Clinical Anatomy & Bedside Clerkship", ar: "التشريح والتدريب السريري" },
      PHC: { en: "Primary Health Care & Preventive Medicine", ar: "الرعاية الصحية الأولية والوقائية" },
      ImD: { en: "Clinical Immunology & Autoimmune Diseases", ar: "علم المناعة السريرية والأمراض" },
      SSC: { en: "Surgical Skills & Life Support", ar: "المهارات الجراحية والدعم الحياتي" },
    };
  ...
  const [activeSubSubject, setActiveSubSubject] = useState<string | null>(() => {
    ...
    return getDefaultSubSubject(subject.id, isRtl);
  });
  const [activeTrack, setActiveTrack] = useState<"Theory" | "Practical" | null>(
    () => {
      if (deepLinkedLecture && deepLinkedLecture.type) {
        return deepLinkedLecture.type === "TBL" ? "Practical" : (deepLinkedLecture.type as any);
      }
      return null;
    },
  );
  ...
  useEffect(() => {
    if (deepLinkedLecture) {
      if (deepLinkedLecture.category) {
        ...
        setActiveSubSubject(getDefaultSubSubject(subject.id, isRtl));
      }
      if (deepLinkedLecture.type) setActiveTrack(deepLinkedLecture.type === "TBL" ? "Practical" : (deepLinkedLecture.type as any));
  ...
  }, [deepLinkedLecture, subject.id, isRtl]);
  ...
  // No translate3d/will-change here: pinning a GPU layer per card (a whole grid
  // of them) exceeds the GPU memory budget on low-end phones and, like the hero
  // layers, gets torn down + rebuilt on every display:none→block tab switch.
  // Transforms are promoted automatically while animating; backface-visibility
  // keeps the promoted layer stable for the duration of the animation.
              ...
              {subSubjects.map((subName) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 ${info.bg}`} />
                    <div className={`absolute -left-10 -bottom-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 delay-100 ${info.bg}`} />
                      <div
                        className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${info.bg} ${info.text} shadow-elevation-1 bg-opacity-80 backdrop-blur-sm`}
                      >
                    ...
                    <div className="mt-5 relative z-10 pointer-events-none flex-1 flex flex-col justify-end">
                      <h3 className="font-display font-semibold text-neutral-900 dark:text-[var(--text-primary)] text-base leading-[1.25] text-balance dark:text-white transition-colors duration-[350ms] antialiased">
                        {subName}
                      </h3>
          ...
          >
            {/* Exactly two rectangular cards designed with the parent subject's size, shape, and color parameters */}
              ...
              >
                {/* Ambient glowing radial background */}
                                        <div className="flex justify-between items-start gap-3 w-full relative z-10 pointer-events-none">
                <div className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 bg-blue-500/10 dark:bg-blue-400/20`} />
                <div className={`absolute -left-10 -bottom-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 delay-100 bg-blue-500/10 dark:bg-blue-400/20`} />
              ...
              >
                {/* Ambient glowing radial background */}
                                        <div className="flex justify-between items-start gap-3 w-full relative z-10 pointer-events-none">
                <div className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 bg-emerald-500/10 dark:bg-emerald-400/20`} />
                <div className={`absolute -left-10 -bottom-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 delay-100 bg-emerald-500/10 dark:bg-emerald-400/20`} />
                  ...
                  (dept) => {
                    ...
                    return (
                      ...
                      >
                        {/* Ambient glowing radial background */}
                                        <div className="flex justify-between items-start gap-3 w-full relative z-10 pointer-events-none">
                        <div className={`absolute -right-10 -top-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 ${dept.bg}`} />
                        <div className={`absolute -left-10 -bottom-10 w-40 h-40 rounded-full blur-[40px] opacity-0 dark:opacity-20 transition-opacity duration-700 delay-100 ${dept.bg}`} />
                          <div
                            className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${dept.bg} text-neutral-700 dark:text-[var(--text-secondary)] shadow-elevation-1 bg-opacity-80 backdrop-blur-sm`}
                          >
