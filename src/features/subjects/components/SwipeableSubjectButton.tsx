...
export const SwipeableSubjectButton = memo(function SwipeableSubjectButton({
  ...
  return (
    <motion.button
      ...
      type="button"
      aria-label={isRtl ? `فتح مادة ${subject.nameAr || subject.name}` : `Open ${subject.name}`}
      className="ios-staggered-card relative group flex flex-col justify-between w-full h-full min-h-[160px] p-5 z-10 bg-white dark:bg-[#1C1C1E] rounded-xl text-left cursor-pointer select-none overflow-hidden"
      style={{
    ...
    >
      ...
      <div className="absolute inset-0 rounded-xl opacity-[0.02] dark:opacity-[0.03] pointer-events-none" style={{ backgroundColor: iconInfo.glow }}></div>
      
      {/* Animated gradient background */}
      <div className="absolute inset-0 rounded-xl opacity-0 pointer-events-none transition-opacity duration-300" style={{ background: `linear-gradient(to bottom right, transparent, ${iconInfo.glow})` }}></div>
      
      {/* Ambient glowing radial background */}
      <div className="absolute inset-0 rounded-xl opacity-[0.035] pointer-events-none transition-opacity duration-300" style={{ background: `radial-gradient(circle at ${isRtl ? "top left" : "top right"}, ${iconInfo.glow} 0%, transparent 80%)` }}></div>
